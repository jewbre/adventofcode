...
function getRegister(input) {
    const register = {};

    for (let order of input) {
        const orderParts = /(\w+) (inc|dec) (-?\d+) if (\w+) ([<>=!]+) (-?\d+)/i.exec(order);

        const changeKey = orderParts[1];
        const value = orderParts[2] === 'inc' ? parseInt(orderParts[3]) : -parseInt(orderParts[3]);
        const margin = parseInt(orderParts[6]);
        const conditionKey = orderParts[4];

        register[changeKey] = register[changeKey] || 0;
        register[conditionKey] = register[conditionKey] || 0;
        switch (orderParts[5]) {
            case '>' :
                if (register[conditionKey] > margin) {
                    register[changeKey] += value;
                }
                break;
            case '<' :
                if (register[conditionKey] < margin) {
        ...
        max = register[changeKey] > max ? register[changeKey] : max;
    }

    return register;
}

function findBiggest(register) {
    let max = -1000000;
...
getRegister(['uz inc 134 if hx > -10',
    'qin dec -300 if h <= 1',
