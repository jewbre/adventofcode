function buildTree(input) {
    ...
    for (let row of input) {
        ...
        if (row.indexOf('->') > 0) {
            // has children
            children = row.substr(row.indexOf('->') + 2).trim().split(', ').map((i) => i.trim());
            row = row.substring(0, row.indexOf('->')).trim();
        }

        let nameParts = row.split(' ');
        let name = nameParts[0].trim();
        let weight = parseInt(nameParts[1].substring(1, nameParts[1].length - 1));

        tree[name] = tree[name] || {};
        tree[name].weight = tree[name].weight || weight;
        tree[name].parent = tree[name].parent || null;
        tree[name].children = children;

        for (let child of children) {
            tree[child] = tree[child] || {};
            tree[child].parent = name || null;
        }
    }
    return tree;
}
...
function balanceTree(tree, leaf) {
    let sum = tree[leaf].weight;

    for (let child of tree[leaf].children) {
        sum += balanceTree(tree, child);
    }

    tree[leaf].subTreeSum = sum;
    return tree[leaf].subTreeSum;
}

function findDiffWeight(tree, leaf, diff) {
    let different = -1;
    let children = tree[leaf].children;
    for (let i = 0; i < children.length; i++) {
        if (tree[children[(i - 1 + children.length) % children.length]].subTreeSum !== tree[children[i]].subTreeSum
            && tree[children[i]].subTreeSum !== tree[children[(i + 1) % children.length]].subTreeSum) {
            different = i;
        }
    }

    if (different === -1) {
        return tree[leaf].weight + diff;
    }

    return findDiffWeight(tree, children[different], tree[children[(different + 1) % children.length]].subTreeSum - tree[children[different]].subTreeSum);
}

function getDiff(input) {
    const tree = buildTree(input);
    const bottom = findBottom(tree);
    balanceTree(tree, bottom);
    // outputTree(tree, bottom, '', 0);
    console.log(findDiffWeight(tree, bottom, 0));
}
...
getDiff(['dihjv (2158) -> gausx, ncdmp, hozgrub',
